...
def replace_text_preserve_format(paragraph, search_text: str, replace_text: str, nth: Union[int, None] = 1) -> int:
    ...
    idx = 0
    while True:
        idx = full_text.find(search_text, idx)
        if idx == -1:
            break
...
def replace_text_in_docx(doc: Document, search_text: str, replace_text: str, nth: Union[int, None] = 1) -> int:
    print(f"[docx_utils] replace_text_in_docx(search_text={search_text}, replace_text={replace_text}, nth={nth}) called")
    """
    在整个文档中查找并替换第 nth 个 search_text（nth=None表示全部替换），最大限度保留格式。
    返回替换的总次数。
    """
    total = 0
    for para in doc.paragraphs:
        total += replace_text_preserve_format(para, search_text, replace_text, nth=None if nth is None else nth-total)
        if nth is not None and total >= nth:
            break
    return total

def replace_text_in_tables(doc: Document, search_text: str, replace_text: str, nth: Union[int, None] = 1) -> int:
    print(f"[docx_utils] replace_text_in_tables(search_text={search_text}, replace_text={replace_text}, nth={nth}) called")
    """
    在所有表格中查找并替换第 nth 个 search_text（nth=None表示全部替换），最大限度保留格式。
    返回替换的总次数。
    """
    total = 0
    for table in doc.tables:
...
def batch_replace(doc: Document, replace_map: dict):
    print(f"[docx_utils] batch_replace(replace_map={replace_map}) called")
    """
    批量查找替换，replace_map={'old1':'new1', ...}
    用法: batch_replace(doc, {'A':'Alpha', 'B':'Beta'})
    """
    for old, new in replace_map.items():
        replace_text_in_docx(doc, old, new, nth=None)
        replace_text_in_tables(doc, old, new, nth=None)
